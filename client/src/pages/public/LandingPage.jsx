...
const TypewriterText = ({ text, delay = 0, speed = 80 }) => {
  ...
  return (
    <>
      {displayedText}
      {started && displayedText.length < text.length && <span className="border-r-2 border-white animate-pulse ml-1"></span>}
    </>
  );
};
      ...
      <div className="relative flex-1 bg-slate-900 overflow-hidden flex flex-col justify-center min-h-[600px]" style={{
        ...
        <div className="relative z-10 px-10 md:px-20 max-w-5xl">
          <h2 className="text-white text-3xl md:text-5xl font-semibold mb-4 tracking-wide shadow-black drop-shadow-md min-h-[48px] md:min-h-[60px]">
            <TypewriterText text="The Best School" delay={0} speed={60} />
          </h2>
          
          <div className="bg-[#1a2538] text-white font-serif font-bold text-5xl md:text-7xl px-8 py-4 inline-block shadow-2xl mb-1 min-h-[88px] md:min-h-[104px]">
            <TypewriterText text="Vidhya Vikas" delay={1200} speed={80} />
          </div>
          
          <br />
          
          <div className="bg-white text-black font-bold text-lg md:text-xl px-8 py-3 inline-block shadow-xl mt-1 min-h-[52px]">
            <TypewriterText text="Empowering Minds, Inspiring Futures" delay={2500} speed={40} />
          </div>
        </div>
