...
const PrincipalDashboard = () => {
  ...
  const stats = data ? [
    { label: 'Total Students', value: data.stats.totalStudents, icon: MdPeople, color: 'from-primary-500 to-indigo-600', sub: 'Currently enrolled' },
    { label: 'Total Staff', value: data.stats.totalStaff, icon: MdPeople, color: 'from-violet-500 to-purple-600', sub: 'Active' },
    { label: 'Pending Approvals', value: data.stats.pendingQuestionsCount, icon: MdQuiz, color: 'from-amber-500 to-orange-600', sub: 'Question papers' },
    { label: 'Open Complaints', value: data.stats.openComplaintsCount, icon: MdReport, color: 'from-red-500 to-rose-600', sub: 'Need attention' },
  ] : [];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              <span className="px-3 py-1 bg-violet-500/20 border border-violet-500/30 rounded-lg text-violet-300 text-xs flex items-center gap-1">
                <div className="w-1.5 h-1.5 bg-violet-400 rounded-full animate-pulse" /> Live Updating
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={fadeInUp} className="lg:col-span-2 rounded-2xl p-5 border border-slate-200" style={{ background: 'rgba(255,255,255,1)' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate('/academic/question-bank')} className="text-primary-500 text-xs hover:text-primary-600 flex items-center gap-1">
              View All <MdArrowForward />
            </button>
          </div>
          <div className="space-y-3">
            {data?.pendingQuestions?.length > 0 ? data.pendingQuestions.map(qp => (
              <div key={qp._id} className="p-4 rounded-xl bg-slate-50 border border-amber-500/10">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-900 text-sm font-medium">{qp.subject}</p>
                    <p className="text-slate-500 text-xs mt-0.5">By {qp.teacherId?.name || 'Unknown'} · {qp.className} · {qp.questions?.length || 0} questions</p>
                    <p className="text-slate-600 text-xs mt-1">Submitted {new Date(qp.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => setApproving({ ...qp, action: 'reject' })}
                      className="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-xs font-medium hover:bg-red-200 flex items-center gap-1 transition-colors">
                      <MdClose /> Reject
                    </button>
                  </div>
                </div>
                {approving?._id === qp._id && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-3 overflow-hidden">
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-slate-900 text-sm placeholder-slate-400 resize-none outline-none focus:border-primary-500/50 mb-2 shadow-sm"
                    />
                    <div className="flex gap-2">
                      <button className={`px-3 py-1.5 rounded-lg text-xs font-medium text-white shadow-sm transition-colors ${approving.action === 'approve' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}>
                        Confirm {approving.action === 'approve' ? 'Approval' : 'Rejection'}
                      </button>
                      <button onClick={() => { setApproving(null); setComment(''); }} className="px-3 py-1.5 bg-slate-200 text-slate-700 rounded-lg text-xs hover:bg-slate-300 transition-colors">Cancel</button>
                    </div>
                  </motion.div>
                )}
              </div>
              ...
              <div className="p-6 text-center text-slate-500 text-sm">
                No pending question papers at the moment.
              </div>
            )}
          </div>
        </motion.div>
        ...
        <motion.div variants={fadeInUp} className="rounded-2xl p-5 border border-slate-200" style={{ background: 'rgba(255,255,255,1)' }}>
            ...
            {feedbackRatings.map(f => (
              <div key={f.facility}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-slate-600 font-medium">{f.facility}</span>
          ...
          <button onClick={() => navigate('/feedback')} className="mt-6 w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-xs rounded-xl transition-colors flex items-center justify-center gap-1 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={fadeInUp} className="rounded-2xl p-5 border border-slate-200" style={{ background: 'rgba(255,255,255,1)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-slate-900 font-semibold">Open Complaints</h3>
            <button onClick={() => navigate('/core')} className="text-primary-500 text-xs hover:text-primary-600 flex items-center gap-1">View all <MdArrowForward /></button>
          </div>
          <div className="space-y-3">
            {data?.openComplaints?.length > 0 ? data.openComplaints.map(c => (
              <div key={c._id} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-slate-200 transition-colors cursor-pointer shadow-sm">
                <div className="w-8 h-8 rounded-lg bg-red-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <MdGavel className="text-red-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-slate-900 text-sm font-medium truncate">{c.title || c.description?.substring(0, 30) + '...'}</p>
                  <p className="text-slate-500 text-xs mt-0.5">{c.studentId?.name || 'Student'} · {c.category}</p>
                </div>
                <div className="flex flex-col gap-1 items-end flex-shrink-0">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${c.priority === 'high' ? priorityColors.high : c.priority === 'medium' ? priorityColors.medium : priorityColors.low}`}>{c.priority}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${statusColors[c.status] || statusColors.open}`}>{c.status.replace('_', ' ')}</span>
                </div>
              </div>
            )) : (
              <div className="p-6 text-center text-slate-500 text-sm">
