...
export default function TeacherDashboard() {
  return (
    <div className="min-h-screen bg-slate-950 p-6 space-y-6">
      ...
      {/* Banner */}
      <motion.div {...fadeInUp}
        className="rounded-2xl p-6 border border-white/5 bg-gradient-to-r from-blue-900 via-indigo-900 to-purple-900 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-purple-600/10" />
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <p className="text-blue-300 text-sm font-medium mb-1">Welcome back,</p>
            <h1 className="text-white text-2xl font-bold">Mrs. Priya Sharma</h1>
            <p className="text-slate-300 text-sm mt-1">Mathematics Dept · Login ID: TCH-2024-047 · 4 Subjects</p>
          </div>
          <div className="flex gap-3">
            <div style={cardStyle} className="rounded-xl px-4 py-2 border border-white/10 text-center">
              <p className="text-white font-bold text-lg">5</p>
              <p className="text-slate-400 text-xs">Classes Today</p>
            </div>
            <div style={cardStyle} className="rounded-xl px-4 py-2 border border-white/10 text-center">
              <p className="text-white font-bold text-lg">12</p>
              <p className="text-slate-400 text-xs">Leave Days</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div {...fadeInUp} style={cardStyle}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-slate-400 text-sm">Sunday, 24 May 2026</span>
          </div>
          <div className="space-y-2">
            {timetable.map((cls, i) => (
              <div key={i} className={`flex items-center gap-4 rounded-xl p-3 bg-white/5 border-l-4 ${cls.color}`}>
                <div className="w-28 shrink-0">
                  <p className="text-slate-300 text-xs font-mono">{cls.time}</p>
                </div>
                <div className="flex-1">
                  <p className="text-white text-sm font-medium">{cls.subject}</p>
                  <p className="text-slate-400 text-xs">{cls.class}</p>
                </div>
                <span className="text-slate-400 text-xs bg-white/5 rounded-lg px-2 py-1">{cls.room}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        {/* Notifications */}
        <motion.div {...fadeInUp} style={cardStyle} className="rounded-2xl p-5 border border-white/5">
            ...
            {questionPapers.map((qp, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                <div className="flex-1 min-w-0">
                  <p className="text-white text-sm font-medium truncate">{qp.title}</p>
                  <p className="text-slate-400 text-xs">{qp.subject} · {qp.date}</p>
                </div>
                <span className={`text-xs px-2 py-1 rounded-lg capitalize ${statusBadge(qp.status)}`}>{qp.status}</span>
                <div className="flex gap-1">
                  {qp.status === 'draft' && (
                    <button className="text-xs bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-2 py-1 transition-colors">Submit</button>
                  )}
                  <button className="text-xs bg-white/10 hover:bg-white/20 text-slate-300 rounded-lg px-2 py-1 transition-colors">View</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {timetable.slice(0, 4).map((cls, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-white/5">
                <div>
                  <p className="text-white text-sm font-medium">{cls.subject} – {cls.class}</p>
                  <p className="text-slate-400 text-xs">{cls.time} · {cls.room}</p>
                </div>
                <button className="flex items-center gap-1 text-xs bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white rounded-lg px-3 py-1.5 transition-all">
