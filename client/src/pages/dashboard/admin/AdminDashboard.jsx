...
const T = ({ active, payload, label }) => {
  if (active && payload?.length) return (
    <div className="bg-slate-800 border border-white/10 rounded-xl px-3 py-2 shadow-xl text-sm">
      ...
      <p className="text-white font-semibold">₹{(payload[0].value / 100000).toFixed(1)}L</p>
    </div>
  );
  return null;
};
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div variants={fadeInUp} initial="initial" animate="animate"
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            ...
            <span className="px-3 py-1 bg-white/10 rounded-lg text-white text-xs">Last login: Today, 9:15 AM</span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={fadeInUp} className="lg:col-span-2 rounded-2xl p-5 border border-white/5" style={{ background: 'rgba(30,41,59,0.8)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-white font-semibold">Fee Collection Overview</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Fee Progress */}
        <motion.div variants={fadeInUp} className="rounded-2xl p-5 border border-white/5" style={{ background: 'rgba(30,41,59,0.8)' }}>
          <h3 className="text-white font-semibold mb-4">Fee Collection Progress</h3>
          <div className="space-y-4">
            {feeProgress.map(f => (
              <div key={f.type}>
                <div className="flex justify-between mb-1.5">
                  <span className="text-slate-300 text-sm">{f.type} Fees</span>
                  <span className="text-white text-sm font-bold">{f.collected}%</span>
                </div>
        ...
        {/* Pending Tasks */}
        <motion.div variants={fadeInUp} className="rounded-2xl p-5 border border-white/5" style={{ background: 'rgba(30,41,59,0.8)' }}>
          <h3 className="text-white font-semibold mb-4">Pending Tasks</h3>
          <div className="space-y-2">
            {pendingTasks.map((t, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/3 border border-white/5 hover:bg-white/5 transition-colors">
                ...
                <p className="text-slate-300 text-sm flex-1 min-w-0 truncate">{t.task}</p>
                <span className="text-slate-500 text-xs flex-shrink-0 bg-white/5 px-2 py-0.5 rounded-lg">{t.module}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <motion.div variants={fadeInUp} className="rounded-2xl p-5 border border-white/5" style={{ background: 'rgba(30,41,59,0.8)' }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white font-semibold">Recent User Activity</h3>
          <span className="text-slate-400 text-xs">{recentLogins.filter(u => u.status === 'online').length} online now</span>
        </div>
            ...
            <tbody className="divide-y divide-white/5">
              {recentLogins.map((u, i) => (
                <tr key={i} className="hover:bg-white/3 transition-colors">
                  ...
                  <td className="py-3 pr-4 text-slate-400 text-xs font-mono">{u.loginId}</td>
                  <td className="py-3 pr-4"><span className={`text-xs px-2 py-0.5 rounded-full ${roleColors[u.role] || 'bg-slate-500/20 text-slate-400'}`}>{u.role}</span></td>
                  <td className="py-3 pr-4 text-slate-400 text-xs">{u.dept}</td>
