    ...
    <>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-1.5 rounded-xl text-sm font-medium transition-all ${tab === t ? "bg-primary-600 text-white" : "bg-slate-800 text-slate-400 hover:text-white"}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-white/5 bg-slate-800/80 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((p, i) => (
              <tr key={p.id} className={`border-b border-white/5 hover:bg-white/5 transition-colors ${i % 2 === 0 ? "bg-slate-900/30" : ""}`}>
        ...
        {showCreate && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-start justify-center overflow-y-auto p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="bg-slate-900 rounded-2xl border border-white/10 w-full max-w-2xl my-8">
              <div className="flex items-center justify-between p-5 border-b border-white/10">
                <h3 className="text-lg font-semibold text-white">Create Question Paper</h3>
                <button onClick={() => setShowCreate(false)} className="p-1.5 hover:bg-white/10 rounded-lg text-slate-400"><MdClose size={20} /></button>
              </div>
              <div className="p-5 space-y-4">
                ...
                <div className="border-t border-white/10 pt-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={addQuestion} className="flex items-center gap-1 text-xs bg-primary-600 hover:bg-primary-500 text-white px-3 py-1.5 rounded-xl"><MdAdd size={14} /> Add Question</button>
                  </div>
                  <div className="space-y-4 max-h-64 overflow-y-auto pr-1">
                    {questions.map((q, qi) => (
                      <div key={q.id} className="bg-slate-800 rounded-xl p-4 border border-white/5">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <select value={q.type} onChange={e => updateQ(q.id, "type", e.target.value)}
                            className="bg-slate-700 border border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:outline-none">
                            {QUESTION_TYPES.map(t => <option key={t}>{t}</option>)}
                          </select>
                          <input type="number" value={q.marks} onChange={e => updateQ(q.id, "marks", e.target.value)}
                            className="w-16 bg-slate-700 border border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:outline-none" placeholder="Marks" />
                        ...
                        <textarea value={q.text} onChange={e => updateQ(q.id, "text", e.target.value)} rows={2}
                          placeholder="Question text..." className="w-full bg-slate-700 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none resize-none mb-2" />
                        {q.type === "MCQ" && (
                            ...
                            {q.options.map((opt, oi) => (
                              <div key={oi} className="flex items-center gap-2">
                                ...
                                <input value={opt} onChange={e => updateOption(q.id, oi, e.target.value)}
                                  placeholder={`Option ${oi + 1}`} className="flex-1 bg-slate-700 border border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:outline-none" />
                              </div>
                            ))}
                          </div>
                        )}
              ...
              <div className="flex gap-3 justify-end p-5 border-t border-white/10">
                <button onClick={() => setShowCreate(false)} className="px-4 py-2 rounded-xl text-sm bg-slate-700 hover:bg-slate-600 text-white">Save as Draft</button>
                <button className="bg-primary-600 hover:bg-primary-500 text-white px-4 py-2 rounded-xl text-sm flex items-center gap-2"><MdSend size={16} /> Submit to Principal</button>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowDetail(null)} className="p-1.5 hover:bg-white/10 rounded-lg text-slate-400"><MdClose size={20} /></button>
              </div>
              <div className="space-y-2 text-sm">
                {[["Subject", showDetail.subject], ["Class", showDetail.class], ["Department", showDetail.department], ["Questions", showDetail.count], ["Created", showDetail.date]].map(([k, v]) => (
                  <div key={k} className="flex justify-between text-slate-300 border-b border-white/5 pb-2">
      ...
      <button onClick={() => setShowCreate(true)}
        className="fixed bottom-8 right-8 bg-primary-600 hover:bg-primary-500 text-white p-4 rounded-2xl shadow-xl flex items-center gap-2 text-sm font-medium transition-all">
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        {filtered.map(p => (
          <div key={p.id} className="rounded-2xl p-5 border border-white/5 bg-slate-800/80">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-white font-semibold">{p.title}</h3>
                <p className="text-slate-400 text-sm mt-1">{p.teacher} · {p.subject} · Class {p.class} · {p.count} Questions</p>
            ...
            {tab === "Pending Review" && (
              <div className="mt-4 flex gap-3">
                <button onClick={() => setConfirm({ paper: p, action: "approve" })}
                  className="flex items-center gap-1.5 bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-xl text-sm"><MdCheckCircle size={16} /> Approve</button>
                <button onClick={() => setConfirm({ paper: p, action: "reject" })}
                  className="flex items-center gap-1.5 bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-xl text-sm"><MdCancel size={16} /> Reject</button>
                <button className="flex items-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-xl text-sm"><MdVisibility size={16} /> View Paper</button>
              </div>
            )}
          </div>
        ))}
      ...
      <AnimatePresence>
        {confirm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="bg-slate-900 rounded-2xl border border-white/10 w-full max-w-md p-6">
              <h3 className="text-lg font-semibold text-white mb-1 capitalize">{confirm.action} Paper?</h3>
              <p className="text-slate-400 text-sm mb-4">"{confirm.paper.title}" by {confirm.paper.teacher}</p>
              <textarea value={comment} onChange={e => setComment(e.target.value)} rows={3}
                placeholder={confirm.action === "reject" ? "Rejection reason (required)" : "Optional comment..."}
                className="w-full bg-slate-800 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none resize-none mb-4" />
              <div className="flex gap-3 justify-end">
                <button onClick={() => setConfirm(null)} className="px-4 py-2 rounded-xl text-sm bg-slate-700 hover:bg-slate-600 text-white">Cancel</button>
                <button onClick={() => setConfirm(null)}
                  className={`px-4 py-2 rounded-xl text-sm text-white ${confirm.action === "approve" ? "bg-green-600 hover:bg-green-500" : "bg-red-600 hover:bg-red-500"}`}>
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            <div className="flex items-center gap-2 text-xs text-slate-500 mb-2">
              ...
              <span className="text-slate-300">Question Bank</span>
            </div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><MdOutlineLibraryBooks className="text-primary-400" /> Question Bank</h1>
            <p className="text-slate-400 text-sm mt-1">{role === "teacher" ? "Manage and submit question papers" : "Review and approve question papers"}</p>
          </div>
        </div>
