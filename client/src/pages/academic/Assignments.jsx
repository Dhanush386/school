  ...
  return (
    <div className="space-y-6">
      <motion.div variants={fadeInUp} initial="initial" animate="animate" className="flex items-start justify-between">
        <div>
          <h1 className="text-slate-900 text-2xl font-bold">Homework</h1>
          <p className="text-slate-500 text-sm mt-1">{isTeacher ? 'Manage and review student homework' : 'Your homework and submissions'}</p>
        </div>
        {isTeacher && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[['Total', assignments.length, 'text-slate-900'], ['Submitted', assignments.filter(a=>a.status==='submitted').length, 'text-green-400'],
          ...
          <div key={l} className="rounded-xl p-4 border border-slate-200" style={{background:'rgba(255,255,255,1)'}}>
        ...
        {assignments.map(a => {
          ...
          return (
            <motion.div key={a.id} variants={staggerItem}
              className="rounded-2xl p-4 border border-slate-200 hover:border-slate-200 transition-all cursor-pointer"
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-500 text-xs mt-1">{a.subject} · Due: {a.dueDate}</p>
        ...
        {showSubmit && selected && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              className="w-full max-w-md rounded-3xl p-6 border border-slate-200"
              style={{ background: 'rgba(15,23,42,0.98)' }}
              onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-slate-900 text-lg font-bold">Submit Assignment</h2>
                <button onClick={() => { setShowSubmit(false); setFileName(''); }} className="text-slate-500 hover:text-slate-900"><MdClose className="text-xl" /></button>
              </div>
              <p className="text-slate-500 text-sm mb-6">{selected.title}</p>
              <div
                onDragOver={e => { e.preventDefault(); setDragging(true); }}
                onDragLeave={() => setDragging(false)}
                onDrop={e => { e.preventDefault(); setDragging(false); const f = e.dataTransfer.files[0]; if (f) setFileName(f.name); }}
                className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${dragging ? 'border-primary-500 bg-primary-500/10' : 'border-slate-200 hover:border-white/20'}`}>
                <MdUpload className="text-4xl text-slate-500 mx-auto mb-3" />
                <p className="text-slate-500 text-sm">{fileName || 'Drag & drop your file here'}</p>
                <p className="text-slate-600 text-xs mt-1">or</p>
                <label className="mt-3 inline-block px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl text-sm cursor-pointer transition-colors">
                  Browse Files
                  <input type="file" className="hidden" onChange={e => setFileName(e.target.files[0]?.name || '')} />
                </label>
              </div>
              <button onClick={handleSubmit} disabled={submitting}
                className="mt-4 w-full py-3 bg-primary-600 hover:bg-primary-500 disabled:opacity-60 text-slate-900 rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2">
